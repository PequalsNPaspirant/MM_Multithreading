	...
	public:
		...
		bool push(T&& obj, const std::chrono::milliseconds& timeout = std::chrono::milliseconds{ 1000 * 60 * 60 }) //default timeout = 1 hr
		{
			...
			do
			{
				...
				localTail = tailProducers_a.load(memory_order_acquire);
			...
			do
			{
				currentHead = headConsumers_a.load(memory_order_acquire);
			}while (currentHead != localHead);
			headConsumers_a.store(localHead + 1, memory_order_release);
		...
		bool pop(T& outVal, const std::chrono::milliseconds& timeout)
		{
			...
			do
			{
				...
				localHead = headConsumers_a.load(memory_order_acquire);
			...
			do
			{
				currentTail = tailProducers_a.load(memory_order_acquire);
			}while (currentTail != localTail);
			tailProducers_a.store(localTail + 1, memory_order_release);
